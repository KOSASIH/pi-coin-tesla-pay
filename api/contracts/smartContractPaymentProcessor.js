...
const smartContractPaymentProcessor = {};

smartContractPaymentProcessor.deploy = async () => {
  const contractCode = `
...
smartContractPaymentProcessor.pay = async (contract, sender, recipient, amount) => {
  const tx = await contract.pay(recipient, amount);
  return tx;
};

export default smartContractPaymentProcessor;
